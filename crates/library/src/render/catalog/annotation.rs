...
impl<'own> CatalogAnnotation<'own> {
    ...
    /// Resolve.
    pub fn resolve(value: &'own Value<WithAnnotations>) -> Self {
        ...
        let (sort_column, sort_ascending) = traverse!(value, "sort")
            .and_then(|sort| match sort {
                Value::Map(map) => Some((
                    map.value.get(&"column".into()).and_then(to_ref_traversal),
                    map.value
                        .get(&"ascending".into())
                ...
                Value::List(_) | Value::Text(_) => Some((to_ref_traversal(sort), true)),
    ...
    /// Sort rows by column.
    pub fn sort(self, rows: &mut Vec<Value<WithAnnotations>>) {
...
fn cmp_rows_by_column<AnnotationsT>(
    row1: &Value<AnnotationsT>,
    row2: &Value<AnnotationsT>,
    column_path: &Traversal<AnnotationsT>,
    ascending: bool,
) -> Ordering {
