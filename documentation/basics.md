...
If you send a `GET` to the page's URL with an `Accept: application/json` header then you will get the JSON with same values that Jinja gets. You can make use of this endpoint with JavaScript to fetch and render the page dynamically with cool widgets. Command line example:

```
curl --header 'Accept: application/json' http://localhost:8000/my-article
```
