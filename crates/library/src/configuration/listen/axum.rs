...
impl TLS {
    ...
    pub async fn to_rustls_config(self) -> io::Result<RustlsConfig> {
        let certificate = self.certificate.as_bytes()?;
        let key = self.key.as_bytes()?;
        RustlsConfig::from_pem(certificate.into(), key.into()).await
    }
}
        ...
        match &self.tls {
            ...
            Some(tls) => {
                ...
                for socket_address in self.to_socket_addrs()? {
                    ...
                    let task = bind_rustls(socket_address, rustls_config.clone()).serve(make_service.clone());
                    tasks.push(Box::pin(task));
                }
            }
        }
