...
///
/// All of the above is handled by attaching a [DeferredResponse] to the *request*. We do this
/// because request mapping middleware cannot return a normal response. Thus, we also need to
/// install [CatchMiddleware], which "catches" the deferment and generates the actual response.
///
...
impl FacadeMiddleware {
    ...
    pub async fn function(State(state_self): State<Self>, mut request: Request) -> Request {
        let uri_path = match request.uri().decoded_path() {
            ...
            Err(_error) => {
                return request.with_deferred_error("decode path".into());
            }
        };
        ...
        if let Some(uri_path) = state_self.configuration.uri.redirect(uri_path) {
            // Note that since RFC 7231 relative `Location` is allowed
            tracing::info!("redirect to: {}", uri_path);
            return request.with_deferred_redirect_to(uri_path.into());
        }

        // Hide

        if state_self.configuration.is_hidden(uri_path) {
        ...
        if let Some(protection) = state_self.configuration.uri.protection(uri_path) {
            if let Some(authenticate) = protection.authorized(request.headers()) {
        ...
        let mut asset_path = state_self.configuration.paths.asset(uri_path);
